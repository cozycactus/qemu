                             ...
                             char *sh_strtable, FILE *output)
{
    ...
    int padding_size = sh_table[data_section_idx]->sh_offset -
                       (sh_table[data_section_idx - 1]->sh_offset +
                        sh_table[data_section_idx - 1]->sh_size) -
                       sh_table[data_section_idx]->sh_addr;
                         ...
                         char *sh_strtable, MemoryRegion *program_mr)
{
    ...
    int bytes_loaded =
        load_image_mr("/tmp/qemu_avr32_tmp_text_sec", program_mr);
    if (bytes_loaded < 0) {
        error_report(
            "[AVR32-BOOT] Unable to load firmware image %s as raw binary",
    ...
    if (file) {
        ...
        if (memcmp(header.e_ident, ELFMAG, SELFMAG) == 0) {
            ...
            if (header.e_machine == EM_AVR32) {
                ...
                sh_strtable = (char *)malloc(
                    sh_table[header.e_shstrndx]->sh_size * sizeof(char));

                avr32_elf_read_sh_string_table(&header, file, sh_table,
                                               sh_strtable);
                avr32_copy_sections(header.e_shnum, file, sh_table, sh_strtable,
                                    program_mr);

            } else {
                error_report(
                    "[AVR32-BOOT] Firmware file is not an AVR32 file!");
            }
        } else {
                         ...
                         MemoryRegion *program_mr, const char *firmware)
{
    ...
    if (avr32_is_elf_file(AVR32_FIRMWARE_FILE)) {
        /*
         * TODO: For some reason QEMU internal ELF-loaders fail to load an AVR32
         * Elf file. For now we use a custom elf-loader to extract the .text
         * section from an elf-file.
         */
